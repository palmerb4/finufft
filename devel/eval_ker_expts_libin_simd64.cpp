...
g++-7 eval_ker_expts_libin_simd64.cpp -o eval_ker_expts_libin_simd64 -Ofast -funroll-loops -march=native -fopt-info
-fopt-info-vec-missed; time ./eval_ker_expts_libin_simd64
...
static inline void evaluate_kernel_vector(FLT *__restrict__ ker, const FLT *__restrict__ args, const FLT beta,
...
int main(int argc, char *argv[]) {
  ...
  int w = 12; // 12, spread width (small), needn't be mult of 4, 15 takes 3.2s but 12 only 0.2s, in g++-7. But not in
              // gcc 5.4.0
...
#pragma omp simd simdlen(64)
  ...
  for (int i = 1; i <= M; ++i) { // changing from i=1 to i=0 slows from 0.2s to 2.4s!!!! I don't understand - has to
                                 // be a better way to control (assembly code?)
