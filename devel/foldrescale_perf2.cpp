/* performance comparison of fold-rescale macro vs function, for spreadinterp.
   ...
   g++-9 -O3 -funroll-loops -march=native -I../include -fopenmp foldrescale_perf2.cpp -o foldrescale_perf2 -lgomp -Ofast
...
alex@fiona /home/alex/numerics/finufft/devel> g++-9 -O3 -funroll-loops -march=native -I../include -fopenmp
foldrescale_perf2.cpp -o foldrescale_perf2 -lgomp alex@fiona /home/alex/numerics/finufft/devel> ./foldrescale_perf2
...
alex@fiona /home/alex/numerics/finufft/devel> g++-9 -O3 -funroll-loops -march=native -I../include -fopenmp
foldrescale_perf2.cpp -o foldrescale_perf2 -lgomp -Ofast -fno-finite-math-only alex@fiona
/home/alex/numerics/finufft/devel> ./foldrescale_perf2 simple array sum:           	0.4 ns/call	(sum:-9554.451222028649)
...
alex@fiona /home/alex/numerics/finufft/devel> g++-9 -O3 -funroll-loops -march=native -I../include -fopenmp
foldrescale_perf2.cpp -o foldrescale_perf2 -lgomp -DNOBIN alex@fiona /home/alex/numerics/finufft/devel>
./foldrescale_perf2 simple array sum:           	1.3 ns/call	(sum:-5028.023988434961) w/ RESCALE1 macro:       	1.3
ns/call	(sum:499984776.5128576) w/ RESCALE macro (pir=0):	6.4 ns/call	(sum:499984776.5128576) w/ RESCALE macro
(pir=1):	1.4 ns/call	(sum:499984776.5128576) w/ foldrescale1:           	7.8 ns/call	(sum:499984776.5128576) w/
foldrescale2:           	6.2 ns/call	(sum:499984776.5128576) w/ foldrescale3:           	6.4 ns/call
(sum:499984776.5128576) w/ foldrescale (pir=0):  	6.3 ns/call	(sum:499984776.5128576) w/ foldrescale (pir=1):  	8.2
ns/call	(sum:499984776.5128576) (ans:0)

alex@fiona /home/alex/numerics/finufft/devel> g++-9 -O3 -funroll-loops -march=native -I../include -fopenmp
foldrescale_perf2.cpp -o foldrescale_perf2 -lgomp -Ofast -fno-finite-math-only -DNOBIN alex@fiona
/home/alex/numerics/finufft/devel> ./foldrescale_perf2 simple array sum:           	0.4 ns/call	(sum:-14573.38274652959)
...
//                        (x*(FLT)M_1_2PI*N + (x*(FLT)M_1_2PI*N<-N/(FLT)2.0 ? (FLT)1.5 : (x*(FLT)M_1_2PI*N>N/(FLT)2.0 ?
//                        (FLT)-0.5 : (FLT)0.5))*N) : \ (x<(FLT)0.0 ? x+(FLT)N : (x>(FLT)N ? x-(FLT)N : x)))
...
//#define RESCALE1(x,N) (x*(FLT)M_1_2PI*N + (x*(FLT)M_1_2PI*N<-N/(FLT)2.0 ? (FLT)1.5*N : (x*(FLT)M_1_2PI*N>N/(FLT)2.0 ?
//(FLT)-0.5*N : (FLT)0.5*N)))
...
#pragma omp for schedule(dynamic, 1000000)
  ...
  for (int pirange = 0; pirange < 2; ++pirange) {
    if (!pirange)
      for (int i = 0; i < M; ++i)
        x[i] = (N / (2 * PI)) * (x[i] + PI); // rescale to [0,N)
    ...
    if (!pirange)
      for (int i = 0; i < M; ++i)
        x[i] = x[i] * ((2 * PI) / N) - PI; // undo rescale
  }
