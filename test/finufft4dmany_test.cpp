...
const char *help[] = {
    "Tester for FINUFFT in 4d, vectorized, all 3 types, either precision.",
    "",
    "Usage: finufft4dmany_test ntrans Nmodes1 Nmodes2 Nmodes3 Nmodes4 Nsrc [tol [debug [spread_thread "
    "[maxbatchsize [spreadsort [upsampfac [errfail]]]]]]]",
    "\teg:\tfinufft4dmany_test 100 20 20 20 20 1e5 1e-3 1 0 0 2 0.0 1e-2",
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 8)
    sscanf(argv[8], "%d", &opts.debug);
  opts.spread_debug = (opts.debug > 1) ? 1 : 0; // see output from spreader
  if (argc > 9)
    sscanf(argv[9], "%d", &opts.spread_thread);
  if (argc > 10)
    sscanf(argv[10], "%d", &opts.maxbatchsize);
  if (argc > 11)
    sscanf(argv[11], "%d", &opts.spread_sort);
  if (argc > 12) {
    sscanf(argv[12], "%lf", &w);
    opts.upsampfac = (FLT)w;
  }
  if (argc > 13)
    sscanf(argv[13], "%lf", &errfail);
...
#pragma omp for schedule(static, TEST_RANDCHUNK)
  ...
  printf("test 4d1 many vs repeated single: ------------------------------------\n");
  CNTime timer;
  timer.start();
  int ier = FINUFFT4D1MANY(ntransf, M, x, y, z, p, c, isign, tol, N1, N2, N3, N4, F, &opts);
  double ti = timer.elapsedsec();
  if (ier > 1) {
    printf("error (ier=%d)!\n", ier);
    return ier;
  } else
    printf("ntr=%d: %lld NU pts to (%lld,%lld,%lld,%lld) modes in %.3g s \t%.3g NU pts/s\n", ntransf, (long long)M,
           (long long)N1, (long long)N2, (long long)N3, (long long)N4, ti, ntransf * M / ti);

  int i = ntransf - 1; // choose a data to check
  BIGINT nt1 = (BIGINT)(0.37 * N1), nt2 = (BIGINT)(0.26 * N2), nt3 = (BIGINT)(-0.39 * N3),
         nt4 = (BIGINT)(-0.39 * N4); // choose some mode index to check
  CPX Ft = CPX(0, 0), J = IMA * (FLT)isign;
  for (BIGINT j = 0; j < M; ++j)
    Ft += c[j + i * M] * exp(J * (nt1 * x[j] + nt2 * y[j] + nt3 * z[j] + nt4 * p[j])); // crude direct
  BIGINT it = N1 / 2 + nt1 + N1 * (N2 / 2 + nt2) + N1 * N2 * (N3 / 2 + nt3) + N1 * N2 * N3 * (N4 / 2 + nt4);           // index in complex F as 1d array
  err = abs(Ft - F[it + i * N]) / infnorm(N, F + i * N);
  errmax = max(err, errmax);
  printf("\tone mode: rel err in F[%lld,%lld,%lld,%lld] of trans#%d is %.3g\n", (long long)nt1, (long long)nt2,
         (long long)nt3, (long long)nt4, i, err);

  // compare the result with FINUFFT4D1
  FFTW_FORGET_WISDOM();
  nufft_opts simpleopts = opts;
  simpleopts.debug = 0; // don't output timing for calls of FINUFFT4D1
  simpleopts.spread_debug = 0;

  CPX *cstart;
  CPX *Fstart;
  CPX *F_4d1 = (CPX *)malloc(sizeof(CPX) * N * ntransf);
  timer.restart();
  for (int k = 0; k < ntransf; ++k) {
    cstart = c + k * M;
    Fstart = F_4d1 + k * N;
    ier = FINUFFT4D1(M, x, y, z, p, cstart, isign, tol, N1, N2, N3, N4, Fstart, &simpleopts);
  }
  ...
  if (ier > 1) {
    ...
    printf("%d of: %lld NU pts to (%lld,%lld,%lld,%lld) modes in %.3g s  \t%.3g NU pts/s\n", ntransf, (long long)M,
           (long long)N1, (long long)N2, (long long)N3, (long long)N4, t, ntransf * M / t);
  printf("\t\t\tspeedup \t T_FINUFFT4D1 / T_finufft4d1many = %.3g\n", t / ti);

  // Check accuracy (worst over the ntransf)
  double maxerror = 0.0;
  for (int k = 0; k < ntransf; ++k)
    maxerror = max(maxerror, (double)relerrtwonorm(N, F_4d1 + k * N, F + k * N));
  errmax = max(maxerror, errmax);
  printf("\tconsistency check: sup ( ||f_many-f||_2 / ||f||_2 ) =  %.3g\n", maxerror);
  free(F_4d1);

  printf("test 4d2 many vs repeated single: ------------------------------------\n");
#pragma omp parallel
  {
    unsigned int se = MY_OMP_GET_THREAD_NUM();
#pragma omp for schedule(static, TEST_RANDCHUNK)
  ...
  FFTW_FORGET_WISDOM();
  timer.restart();
  ier = FINUFFT4D2MANY(ntransf, M, x, y, z, p, c, isign, tol, N1, N2, N3, N4, F, &opts);
  ti = timer.elapsedsec();
  if (ier > 1) {
    printf("error (ier=%d)!\n", ier);
    return ier;
  } else
    printf("ntr=%d: (%lld,%lld,%lld,%lld) modes to %lld NU pts in %.3g s \t%.3g NU pts/s\n", ntransf, (long long)N1,
           (long long)N2, (long long)N3, (long long)N4, (long long)M, ti, ntransf * M / ti);
    ...
    for (BIGINT m3 = -(N3 / 2); m3 <= (N3 - 1) / 2; ++m3) {
      for (BIGINT m2 = -(N2 / 2); m2 <= (N2 - 1) / 2; ++m2) { // loop in correct order over F
        for (BIGINT m1 = -(N1 / 2); m1 <= (N1 - 1) / 2; ++m1) {
          ct += F[i * N + m++] * exp(J * (m1 * x[jt] + m2 * y[jt] + m3 * z[jt] + m4 * p[jt])); // crude direct
        }
      }
    }
  }
  ...
  FFTW_FORGET_WISDOM();
  // compare the result with FINUFFT4D2...
  CPX *c_4d2 = (CPX *)malloc(sizeof(CPX) * M * ntransf);
  timer.restart();
  for (int k = 0; k < ntransf; ++k) {
    cstart = c_4d2 + k * M;
    Fstart = F + k * N;
    ier = FINUFFT4D2(M, x, y, z, p, cstart, isign, tol, N1, N2, N3, N4, Fstart, &simpleopts);
  }
  t = timer.elapsedsec();
  if (ier > 1) {
    printf("error (ier=%d)!\n", ier);
    ...
    printf("%d of: (%lld,%lld,%lld,%lld) modes to %lld NU pts in %.3g s \t%.3g NU pts/s\n", ntransf, (long long)N1,
           (long long)N2, (long long)N3, (long long)N4, (long long)M, t, ntransf * M / t);
  printf("\t\t\tspeedup \t T_FINUFFT4D2 / T_finufft4d2many = %.3g\n", t / ti);

  maxerror = 0.0; // worst error over the ntransf
  for (int k = 0; k < ntransf; ++k)
    maxerror = max(maxerror, (double)relerrtwonorm(M, c_4d2 + k * M, c + k * M));
  errmax = max(maxerror, errmax);
  printf("\tconsistency check: sup ( ||c_many-c||_2 / ||c||_2 ) =  %.3g\n", maxerror);
  free(c_4d2);

  printf("test 4d3 many vs repeated single: ------------------------------------\n");
  FFTW_FORGET_WISDOM();
...
#pragma omp for schedule(static, TEST_RANDCHUNK)
  ...
  timer.restart();
  ier = FINUFFT4D3MANY(ntransf, M, x, y, z, p, c, isign, tol, N, s_freq, t_freq, u_freq, v_freq, F, &opts);
  ti = timer.elapsedsec();
  if (ier > 1) {
    printf("error (ier=%d)!\n", ier);
    return ier;
  ...
  Ft = CPX(0, 0);
  for (BIGINT j = 0; j < M; ++j)
    Ft += c[i * M + j] * exp(J * (s_freq[kt] * x[j] + t_freq[kt] * y[j] + u_freq[kt] * z[j] + v_freq[kt] * p[j]));
  err = abs(Ft - F[kt + i * N]) / infnorm(N, F + i * N);
  errmax = max(err, errmax);
  printf("\t one targ: rel err in F[%lld] of trans#%d is %.3g\n", (long long)kt, i, err);

  FFTW_FORGET_WISDOM();
  // compare the result with FINUFFT4D3...
  CPX *f_4d3 = (CPX *)malloc(sizeof(CPX) * N * ntransf);
  timer.restart();
  for (int k = 0; k < ntransf; ++k) {
    Fstart = f_4d3 + k * N;
    cstart = c + k * M;
    ier = FINUFFT4D3(M, x, y, z, p, cstart, isign, tol, N, s_freq, t_freq, u_freq, v_freq, Fstart, &simpleopts);
  }
  t = timer.elapsedsec();
  if (ier > 1) {
    printf("error (ier=%d)!\n", ier);
    ...
    printf("%d of: %lld NU to %lld NU in %.3g s   \t%.3g tot NU pts/s\n", ntransf, (long long)M, (long long)N, t,
           ntransf * (M + N) / t);
  printf("\t\t\tspeedup \t T_FINUFFT4D3 / T_finufft4d3many = %.3g\n", t / ti);

  maxerror = 0.0; // worst error over the ntransf
  for (int k = 0; k < ntransf; ++k)
    maxerror = max(maxerror, (double)relerrtwonorm(N, f_4d3 + k * N, F + k * N));
  errmax = max(maxerror, errmax);
  printf("\tconsistency check: sup ( ||f_many-f||_2 / ||f||_2 ) =  %.3g\n", maxerror);
  free(f_4d3);
