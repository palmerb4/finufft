...
const char *help[] = {"Tester for FINUFFT in 1d, all 3 types, either precision.",
                      "",
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 4)
    sscanf(argv[4], "%d", &opts.debug);
  opts.spread_debug = (opts.debug > 1) ? 1 : 0; // see output from spreader
  if (argc > 5)
    sscanf(argv[5], "%d", &opts.spread_sort);
  if (argc > 6) {
    sscanf(argv[6], "%lf", &w);
    opts.upsampfac = (FLT)w;
  }
  if (argc > 7)
    sscanf(argv[7], "%lf", &errfail);
...
#pragma omp for schedule(static, TEST_RANDCHUNK) // static => non-stochastic
  ...
  //#pragma omp declare reduction (cmplxadd:CPX:omp_out=omp_out+omp_in) initializer(omp_priv={0.0,0.0})  // only for
  // openmp v 4.0! #pragma omp parallel for schedule(static,TEST_RANDCHUNK) reduction(cmplxadd:Ft)
  FLT Ftr = 0.0, Fti = 0.0;
