...
void dirft4d1(BIGINT nj, FLT *x, FLT *y, FLT *z, FLT *p, CPX *c, int iflag, BIGINT ms, BIGINT mt, BIGINT mu, BIGINT mv,
              CPX *f)
/* Direct computation of 4D type-1 nonuniform FFT. Interface same as finufft4d1.
c                        nj-1
c     f[k1,k2,k3,k4] =   SUM  c[j] exp(+-i (k1 x[j] + k2 y[j] + k3 z[j] + k4 p[j]))
c                        j=0
c
...
*/
{
  BIGINT k1min = -(ms / 2), k2min = -(mt / 2), k3min = -(mu / 2), k4min = -(mv / 2); // integer divide
  BIGINT N = ms * mt * mu * mv;                                                      // total # output modes
...
void dirft4d2(BIGINT nj, FLT *x, FLT *y, FLT *z, FLT *p, CPX *c, int iflag, BIGINT ms, BIGINT mt, BIGINT mu, BIGINT mv,
              CPX *f)
/* Direct computation of 4D type-2 nonuniform FFT. Interface same as finufft4d2

     c[j] =   SUM    f[k1,k2,k3,k3] exp(+-i (k1 x[j] + k2 y[j] + k3 z[j] + k4 p[j]))
          k1,k2,k3,k4
                            for j = 0,...,nj-1
    ...
    k2 ordering (medium), then increasing k3 (fast), then increasing k4 (fast).
...
void dirft4d3(BIGINT nj, FLT *x, FLT *y, FLT *z, FLT *p, CPX *c, int iflag, BIGINT nk, FLT *s, FLT *t, FLT *u, FLT *v,
              CPX *f)
/* Direct computation of 4D type-3 nonuniform FFT. Interface same as finufft4d3
c               nj-1
c     f[k]  =   SUM   c[j] exp(+-i (s[k] x[j] + t[k] y[j] + u[k] z[j] + v[k] p[j]))
c               j=0
c                    for k = 0, ..., nk-1
...
*/
{
  for (BIGINT k = 0; k < nk; ++k) {
    ...
    f[k] = CPX(0, 0);
    for (BIGINT j = 0; j < nj; ++j)
      f[k] += c[j] * exp(ss * x[j] + tt * y[j] + uu * z[j] + vv * p[j]);
  }
}
