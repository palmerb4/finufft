...
*/
{
  ...
  for (int k = 0; k < N; ++k)
    s[k] = 10 * cos(1.2 * k); // normal-sized coords
  FLT *shuge = (FLT *)malloc(sizeof(FLT) * N);
  FLT huge = 1e11; // no smaller than MAX_NF
  for (int k = 0; k < N; ++k)
    shuge[k] = huge * s[k]; // some huge coords
  ...
  for (int k = 0; k < N; ++k)
    shuge[k] = sqrt(huge) * s[k]; // less huge coords
  ...
  for (int j = 0; j < M * ndata; ++j)
    cm[j] = sin((FLT)1.3 * j) + IMA * cos((FLT)0.9 * j); // reset cm for 3d1many
