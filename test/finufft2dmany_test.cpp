...
const char *help[] = {"Tester for FINUFFT in 2d, vectorized, all 3 types, either precision.",
                      "",
                      "Usage: finufft2dmany_test ntrans Nmodes1 Nmodes2 Nsrc [tol [debug [spread_thread [maxbatchsize "
                      "[spreadsort [upsampfac [errfail]]]]]]]",
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 6)
    sscanf(argv[6], "%d", &opts.debug);
  opts.spread_debug = (opts.debug > 1) ? 1 : 0; // see output from spreader
  if (argc > 7)
    sscanf(argv[7], "%d", &opts.spread_thread);
  if (argc > 8)
    sscanf(argv[8], "%d", &opts.maxbatchsize);
  if (argc > 9)
...
#pragma omp for schedule(static, TEST_RANDCHUNK)
  ...
  if (ier > 1) {
    ...
    printf("ntr=%d: %lld NU pts to (%lld,%lld) modes in %.3g s \t%.3g NU pts/s\n", ntransf, (long long)M, (long long)N1,
           (long long)N2, ti, ntransf * M / ti);
...
#pragma omp for schedule(static, TEST_RANDCHUNK)
  ...
  if (ier > 1) {
    ...
    printf("ntr=%d: (%lld,%lld) modes to %lld NU pts in %.3g s \t%.3g NU pts/s\n", ntransf, (long long)N1,
           (long long)N2, (long long)M, ti, ntransf * M / ti);
