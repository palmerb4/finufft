...
const char *help[] = {"Tester for FINUFFT in 1d, vectorized, all 3 types, either precision.",
                      "",
                      "Usage: finufft1dmany_test ntrans Nmodes Nsrc [tol [debug [spread_thread [maxbatchsize "
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 5)
    sscanf(argv[5], "%d", &opts.debug);
  opts.spread_debug = (opts.debug > 1) ? 1 : 0; // see output from spreader
  if (argc > 6)
    sscanf(argv[6], "%d", &opts.spread_thread);
  if (argc > 7)
    sscanf(argv[7], "%d", &opts.maxbatchsize);
  if (argc > 8)
...
#pragma omp for schedule(static, TEST_RANDCHUNK)
    for (BIGINT j = 0; j < M; ++j)
      x[j] = 2.0 + PI * randm11r(&se); // new x_j srcs
  }
