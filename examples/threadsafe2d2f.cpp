/* This is a 2D type-2 demo calling single-threaded FINUFFT inside an OpenMP
   ...
   g++ -fopenmp threadsafe2d2f.cpp -I../include ../lib/libfinufft.so -o threadsafe2d2f -lfftw3 -lfftw3_threads -lm -g
   -Wall
...
// self-contained small test that single-prec FINUFFT2D2 no error nor crash
{
  ...
  int ier = finufftf2d2(n_read * n_spokes, x.data(), y.data(), ksp.data(), -1, 1e-3, n_rows, n_cols, img.data(), opts);
