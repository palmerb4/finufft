      ...
      do k5 = -mw/2, (mw-1)/2
         do k4 = -mv/2, (mv-1)/2
               do k3 = -mu/2, (mu-1)/2
                  do k2 = -mt/2, (mt-1)/2
                     do k1 = -ms/2, (ms-1)/2
                           fk(k1,k2,k3,k4,k5) = dcmplx(0d0,0d0)
                     enddo
                  enddo
               enddo
         enddo
...
c     ----------------------------------------------------------
         ...
         do k1 = 1, (mv-1)/2
            z4n(k1) = zf*z4n(k1-1)
            z4n(-k1)= dconjg(z4n(k1))
         enddo
         if (mv/2*2.eq.mv) z4n(-mv/2) = dconjg(zf*z4n(mv/2-1))
c
c     ----------------------------------------------------------
...
************************************************************************
      integer j, k1, k2, k3, k4, k5
      complex*16 zf, cm1,cm2,cm3,cm4,cm5,z1n(-ms/2:(ms-1)/2),
     $          z2n(-mt/2:(mt-1)/2),z3n(-mu/2:(mu-1)/2),
     $          z4n(-mv/2:(mv-1)/2)
c
      do j = 1, nj
...
************************************************************************
     ...
     &                     nk,sk,tk,uk,vk,wk,fk)
      implicit none
      integer nj, iflag, nk
      real*8 xj(nj),yj(nj),zj(nj),pj(nj),qj(nj),sk(nk),tk(nk),
     $       uk(nk),vk(nk),wk(nk)
...
c
     ...
     &        ( dcos(ssk*xj(j)+stk*yj(j)+suk*zj(j)+svk*pj(j)+swk*qj(j)),
     &          dsin(ssk*xj(j)+stk*yj(j)+suk*zj(j)+svk*pj(j)+swk*qj(j)))
         enddo
      enddo
      end
