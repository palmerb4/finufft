...
class Plan:
    ...
    Args:
        ...
        n_modes_or_dim  (int or tuple of ints): if ``nufft_type`` is 1 or 2,
                        ...
                        number of dimensions (between 1 and 5).
    ...
    def __init__(self, nufft_type, n_modes_or_dim, n_trans=1, eps=1e-6, isign=None, **kwargs):
        ...
        n_modes = np.ones([5], dtype=np.int64)
        if nufft_type == 3:
            ...
            if npmodes.size > 5 or npmodes.size < 1:
                raise RuntimeError(
                    "FINUFFT n_modes dimension must be 1, 2, 3, 4, or 5")
            dim = int(npmodes.size)
            n_modes[0:dim] = npmodes[::-1]

        n_modes = (c_longlong * 5)(*n_modes)
    ...
    # setpts
    def setpts(self, x=None, y=None, z=None, p=None, q=None, s=None, t=None, u=None, v=None, w=None):
        r"""
        Set the nonuniform points
        ...
        Args:
            ...
            z       (float[M], optional): third coordinate of the nonuniform
                    points (source for type 1 and 3, target for type 2).
            p       (float[M], optional): fourth coordinate of the nonuniform
                    points (source for type 1 and 3, target for type 2).
            q       (float[M], optional): fifth coordinate of the nonuniform
                    points (source for type 1 and 3, target for type 2).
            ...
            u       (float[N], optional): third coordinate of the nonuniform
                    points (target for type 3).
            v       (float[N], optional): fourth coordinate of the nonuniform
                    points (target for type 3).
            w       (float[N], optional): fifth coordinate of the nonuniform
                    points (target for type 3).
        """
        if self.is_single:
            # array sanity check
        ...
        dim = self.dim
        tp = self.type
        (self.nj, self.nk) = valid_setpts(tp, dim, self._xj, self._yj, self._zj,
                                          self._pj, self._qj, self._s, self._t,
                                          self._u, self._v, self._w)

        # call set pts for single prec plan
        if self.dim == 1:
            ier = self._setpts(self.inner_plan, self.nj, self._xj,
                               self._yj, self._zj, self._pj, self._qj, self.nk, self._s, self._t, self._u, self._v, self._w)
        elif self.dim == 2:
            ier = self._setpts(self.inner_plan, self.nj, self._yj,
                               self._xj, self._zj, self._pj, self._qj, self.nk, self._t, self._s, self._u, self._v, self._w)
        elif self.dim == 3:
            ier = self._setpts(self.inner_plan, self.nj, self._zj,
                               self._yj, self._xj, self._pj, self._qj, self.nk, self._u, self._t, self._s, self._v, self._w)
        elif self.dim == 4:
            ier = self._setpts(self.inner_plan, self.nj, self._pj, self._zj,
                               self._yj, self._xj, self._qj, self.nk, self._v, self._u, self._t, self._s, self._w)
        elif self.dim == 5:
            ier = self._setpts(self.inner_plan, self.nj, self._qj, self._pj, self._zj,
                               self._yj, self._xj, self.nk, self._w, self._v, self._u, self._t, self._s)
        else:
            raise RuntimeError("FINUFFT dimension must be 1, 2, 3, 4, or 5")
    ...
    def execute(self, data, out=None):
        ...
        if tp == 2:
            valid_fshape(data.shape, n_trans, dim, ms, mt, mu, mv, mw, None, 2)
        else:
        ...
        if out is not None:
            if tp == 1:
                valid_fshape(out.shape, n_trans, dim,
                             ms, mt, mu, mv, mw, None, 1)
            if tp == 2:
                valid_cshape(out.shape, nj, n_trans)
            if tp == 3:
                valid_fshape(out.shape, n_trans, dim, None,
                             None, None, None, None, nk, 3)
        ...
        if out is None:
            ...
            if tp == 1:
                _out = np.squeeze(
                    np.zeros([n_trans, mw, mv, mu, mt, ms], dtype=pdtype, order='C'))
            if tp == 2:
                _out = np.squeeze(
                    np.zeros([n_trans, nj], dtype=pdtype, order='C'))
            if tp == 3:
                _out = np.squeeze(
                    np.zeros([n_trans, nk], dtype=pdtype, order='C'))
...
def _rchk(x):
    ...
    if x is not None and x.dtype is not np.dtype('float64'):
        raise RuntimeError(
            'FINUFFT data type must be float64 for double precision, data may have mixed precision types')
...
def _cchk(x):
    ...
    if x is not None and (x.dtype is not np.dtype('complex128') and x.dtype is not np.dtype('float64')):
        raise RuntimeError(
            'FINUFFT data type must be complex128 for double precision, data may have mixed precision types')
...
def _rchkf(x):
    ...
    if x is not None and x.dtype is not np.dtype('float32'):
        raise RuntimeError(
            'FINUFFT data type must be float32 for single precision, data may have mixed precision types')
...
def _cchkf(x):
    ...
    if x is not None and (x.dtype is not np.dtype('complex64') and x.dtype is not np.dtype('float32')):
        raise RuntimeError(
            'FINUFFT data type must be complex64 for single precision, data may have mixed precision types')
...
def valid_setpts(tp, dim, x, y, z, p, q, s, t, u, v, w):
    if x.ndim != 1:
    ...
    if dim > 3:
        if p.ndim != 1:
            raise RuntimeError('FINUFFT p must be a vector')
        if p.size != nj:
            raise RuntimeError('FINUFFT p must have same length as x')
        if tp == 3:
            if v.ndim != 1:
                raise RuntimeError('FINUFFT v must be a vector')
            if v.size != nk:
                raise RuntimeError('FINUFFT v must have same length as s')

    if dim > 4:
        if q.ndim != 1:
            raise RuntimeError('FINUFFT q must be a vector')
        if q.size != nj:
            raise RuntimeError('FINUFFT q must have same length as x')
        if tp == 3:
            if w.ndim != 1:
                raise RuntimeError('FINUFFT w must be a vector')
            if w.size != nk:
                raise RuntimeError('FINUFFT w must have same length as s')
...
def valid_ntr_tp12(dim, shape, n_transin, n_modesin):
    if len(shape) == dim+1:
        ...
        raise RuntimeError(
            'FINUFFT type 1 output dimension or type 2 input dimension must be either dim or dim+1(n_trans>1)')

    if n_transin is not None and n_trans != n_transin:
        raise RuntimeError(
            'FINUFFT input n_trans and output n_trans do not match')

    if n_modesin is not None:
        if n_modes != n_modesin:
            raise RuntimeError(
                'FINUFFT input n_modes and output n_modes do not match')
...
def valid_cshape(cshape, xsize, n_trans):
    if n_trans == 1:
        ...
        if cshape[0] != xsize:
            raise RuntimeError(
                'FINUFFT c.size must be same as x.size if n_trans = 1')
    if n_trans > 1:
        if len(cshape) != 2:
...
# valid shape of f
def valid_fshape(fshape, n_trans, dim, ms, mt, mu, mv, mw, nk, tp):
    if tp == 3:
        if n_trans == 1:
            if len(fshape) != 1:
                raise RuntimeError(
                    'FINUFFT f.ndim must be 1 for type 3 if n_trans = 1')
            if fshape[0] != nk:
                raise RuntimeError(
                    'FINUFFT f.size of must be nk if n_trans = 1')
        if n_trans > 1:
            if len(fshape) != 2:
                raise RuntimeError(
                    'FINUFFT f.ndim must be 2 for type 3 if n_trans > 1')
        ...
        if n_trans == 1:
            if len(fshape) != dim:
                raise RuntimeError(
                    'FINUFFT f.ndim must be same as the problem dimension for type 1 or 2 if n_trans = 1')
        if n_trans > 1:
            if len(fshape) != dim+1:
                raise RuntimeError(
        ...
        if fshape[-1] != ms:
            raise RuntimeError(
                'FINUFFT f.shape is not consistent with n_modes')
        if dim > 1:
            if fshape[-2] != mt:
                raise RuntimeError(
                    'FINUFFT f.shape is not consistent with n_modes')
...
def is_single_dtype(dtype):
    ...
    if dtype == np.dtype('float64') or dtype == np.dtype('complex128'):
        ...
        raise RuntimeError(
            'FINUFFT dtype(precision type) must be single or double')
...
def setkwopts(opt, **kwargs):
    ...
    for key, value in kwargs.items():
        if hasattr(opt, key):
            ...
            warnings.warn(
                'Warning: nufft_opts does not have attribute "' + key + '"', Warning)
...
def invoke_guru(dim, tp, x, y, z, p, q, c, s, t, u, v, w, f, isign, eps, n_modes, **kwargs):
    # infer dtype from x
    ...
    if n_modes is not None:
        if (not isinstance(n_modes, tuple)) and (not isinstance(n_modes, numbers.Integral)):
            raise RuntimeError(
                'FINUFFT input n_modes must be a tuple or an integer')
    ...
    if isinstance(n_modes, tuple):
        ...
        if (not all(isinstance(elmi, numbers.Integral) for elmi in n_modes)):
            raise RuntimeError(
                'FINUFFT all elements of input n_modes must be integer')
    ...
    if tp == 3:
        ...
        plan = Plan(tp, n_modes, n_trans, eps, isign,
                    **dict(kwargs, dtype=pdtype))
...
{target_pts_generate}
    ...
    pts = ('x', 'y', 'z', 'p', 'q')
    target_pts = ('s', 't', 'u', 'v', 'w')
    sample_modes = (50, 75, 100, 10, 20)
    ...
    v['pt_inner'] = ' + '.join('k{0} {1}(j)'.format(i, x)
                               for i, x in zip(dims, pts[:dim]))
    v['pt_constraint'] = ', '.join(
        '-N{0}/2 <= k{0} <= (N{0}-1)/2'.format(i) for i in dims)
    v['pts_doc'] = '\n'.join(
        '      {}         (float[M]): nonuniform points, valid only in [-3pi, 3pi].'.format(x) for x in pts[:dim])
    ...
    # for type 3 only
    v['src_pts_doc'] = '\n'.join(
        '      {}         (float[M]): nonuniform source points.'.format(x) for x in pts[:dim])
    v['target_pts_doc'] = '\n'.join(
        '      {}         (float[N]): nonuniform target points.'.format(x) for x in target_pts[:dim])
    v['pt_inner_type3'] = ' + '.join('{0}[k] {1}[j]'.format(s, x)
                                     for s, x in zip(target_pts[:dim], pts[:dim]))

    # for type 3 example only
    v['target_pts'] = ', '.join(str(x) for x in target_pts[:dim])
    v['target_pts_generate'] = '\n'.join(
        '      {} = 2 * np.pi * np.random.uniform(size=N)'.format(x) for x in target_pts[:dim])
...
def nufft1d1(x, c, n_modes=None, out=None, eps=1e-6, isign=1, **kwargs):
    return invoke_guru(1, 1, x, None, None, None, None, c, None, None, None, None, None, out, isign, eps, n_modes, **kwargs)


# 1d2
def nufft1d2(x, f, out=None, eps=1e-6, isign=-1, **kwargs):
    return invoke_guru(1, 2, x, None, None, None, None, out, None, None, None, None, None, f, isign, eps, None, **kwargs)


# 1d3
def nufft1d3(x, c, s, out=None, eps=1e-6, isign=1, **kwargs):
    return invoke_guru(1, 3, x, None, None, None, None, c, s, None, None, None, None, out, isign, eps, None, **kwargs)


# 2d1
def nufft2d1(x, y, c, n_modes=None, out=None, eps=1e-6, isign=1, **kwargs):
    return invoke_guru(2, 1, x, y, None, None, None, c, None, None, None, None, None, out, isign, eps, n_modes, **kwargs)


# 2d2
def nufft2d2(x, y, f, out=None, eps=1e-6, isign=-1, **kwargs):
    return invoke_guru(2, 2, x, y, None, None, None, out, None, None, None, None, None, f, isign, eps, None, **kwargs)


# 2d3
def nufft2d3(x, y, c, s, t, out=None, eps=1e-6, isign=1, **kwargs):
    return invoke_guru(2, 3, x, y, None, None, None, c, s, t, None, None, None, out, isign, eps, None, **kwargs)


# 3d1
def nufft3d1(x, y, z, c, n_modes=None, out=None, eps=1e-6, isign=1, **kwargs):
    return invoke_guru(3, 1, x, y, z, None, None, c, None, None, None, None, None, out, isign, eps, n_modes, **kwargs)


# 3d2
def nufft3d2(x, y, z, f, out=None, eps=1e-6, isign=-1, **kwargs):
    return invoke_guru(3, 2, x, y, z, None, None, out, None, None, None, None, None, f, isign, eps, None, **kwargs)


# 3d3
def nufft3d3(x, y, z, c, s, t, u, out=None, eps=1e-6, isign=1, **kwargs):
    return invoke_guru(3, 3, x, y, z, None, None, c, s, t, u, None, None, out, isign, eps, None, **kwargs)


# 4d1
def nufft4d1(x, y, z, p, c, n_modes=None, out=None, eps=1e-6, isign=1, **kwargs):
    return invoke_guru(4, 1, x, y, z, p, None, c, None, None, None, None, None, out, isign, eps, n_modes, **kwargs)


# 4d2
def nufft4d2(x, y, z, p, f, out=None, eps=1e-6, isign=-1, **kwargs):
    return invoke_guru(4, 2, x, y, z, p, None, out, None, None, None, None, None, f, isign, eps, None, **kwargs)


# 4d3
def nufft4d3(x, y, z, p, c, s, t, u, v, out=None, eps=1e-6, isign=1, **kwargs):
    return invoke_guru(4, 3, x, y, z, p, None, c, s, t, u, v, None, out, isign, eps, None, **kwargs)


# 5d1
def nufft5d1(x, y, z, p, q, c, n_modes=None, out=None, eps=1e-6, isign=1, **kwargs):
    return invoke_guru(5, 1, x, y, z, p, q, c, None, None, None, None, None, out, isign, eps, n_modes, **kwargs)


# 5d2
def nufft5d2(x, y, z, p, q, f, out=None, eps=1e-6, isign=-1, **kwargs):
    return invoke_guru(5, 2, x, y, z, p, q, out, None, None, None, None, None, f, isign, eps, None, **kwargs)


# 5d3
def nufft5d3(x, y, z, p, q, c, s, t, u, v, w, out=None, eps=1e-6, isign=1, **kwargs):
    return invoke_guru(5, 3, x, y, z, p, q, c, s, t, u, v, w, out, isign, eps, None, **kwargs)

_set_nufft_doc(nufft1d1, 1, 1,
               'python/examples/simple1d1.py, python/examples/simpleopts1d1.py')
_set_nufft_doc(nufft1d2, 1, 2)
_set_nufft_doc(nufft1d3, 1, 3)
_set_nufft_doc(nufft2d1, 2, 1, 
               'python/examples/simple2d1.py, python/examples/many2d1.py')
...
_set_nufft_doc(nufft4d1, 4, 1)
_set_nufft_doc(nufft4d2, 4, 2)
_set_nufft_doc(nufft4d3, 4, 3)
_set_nufft_doc(nufft5d1, 5, 1)
_set_nufft_doc(nufft5d2, 5, 2)
_set_nufft_doc(nufft5d3, 5, 3)
