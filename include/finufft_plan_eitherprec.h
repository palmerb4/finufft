...
typedef struct FINUFFT_PLAN_S { // the main plan struct; note C-compatible struct
  ...
  int dim;       // overall dimension: 1,2,3,4, or 5
  ...
  BIGINT mv; // number of modes in v (4) direction = N4
  BIGINT mw; // number of modes in w (5) direction = N5
  ...
  FLT *X, *Y, *Z, *P, *Q; // for t1,2: ptr to user-supplied NU pts (no new allocs).
                          // for t3: allocated as "primed" (scaled) src pts x'_j, etc

  // type 3 specific
  FLT *S, *T, *U, *V, *W;      // pointers to user's target NU pts arrays (no new allocs)
  ...
  FLT *Sp, *Tp, *Up, *Vp, *Wp; // internal primed targs (s'_k, etc), allocated
