...
function c = finufft4d2(x,y,z,p,isign,eps,f,o)

if nargin<8, o.dummy=1; end
valid_setpts(2,4,x,y,z,p,[]);
o.floatprec=class(x);                      % should be 'double' or 'single'
[ms,mt,mu,mv,n_transf] = size(f);          % if f 4D array, n_transf=1
plan = finufft_plan(2,[ms;mt;mu;mv],isign,n_transf,eps,o);
plan.setpts(x,y,z,p,[]);
c = plan.execute(f);
